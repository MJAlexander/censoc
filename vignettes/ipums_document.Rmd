...
This variables is a unique identifier that allows correspondence across the two datasets. Additionally, the `SEX` variable is needed in order to restrict the sample to be males only.  
To obtain these variables:

1. From the PERSON drop down, select 'TECHNICAL'. Note that `HISTID` should be `[preselected]`, but you can click on the cross next to it just to make sure. 
...
The unique ID `HISTID` is available in the CenSoc and IPUMS census dataset and can be used to merge the two datasets. 
...
Merge us the [IPUMSR Package](https://cran.r-project.org/web/packages/ipumsr/vignettes/ipums.html): 
